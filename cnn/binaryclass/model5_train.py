...
# saved_model_filename = 'cnn/binaryclass/bc_model5.tar'
...
saved_model_filename = f'cnn/binaryclass/bc_model5_{cell_width}.tar'
...
BATCH_SIZE_uniform = 16  # model 5: truly random grid cells (no anchor guaranteed), unrestricted
                          # by hwy_40/candidate geometry -- gives the network direct training
                          # exposure to "ordinary cell, no highway nearby" windows, which real+
                          # candidate anchors alone never produce (see conversation notes)
BATCH_SIZE = BATCH_SIZE_real + BATCH_SIZE_fill + BATCH_SIZE_random + BATCH_SIZE_uniform
...
frac_train_uniform = 0.7  # fraction of uniform-random grid cells to use for training
...
def extract_patch_from_arrays(feature_array, label_array, row, col, window,
    ...
    # model 5: the anchor cell (row, col) can land anywhere in the window, not just the
    # central sv x sv "potential" block. This is what lets an *ordinary* (non-anchor) cell
    # end up sitting at the window's own geometric center during training -- the exact
    # configuration prediction uses for every cell, which the old central-block-only jitter
    # never produced. class_idx is kept for backward compatibility (unused downstream) but
    # is no longer a meaningful sv x sv class index once jitter spans the full window.
    central_start = 0
    central_end   = window
...
# model 5: truly uniform-random grid cells -- every grid_id, unrestricted by hwy_40/candidate
# ray geometry. This is the anchor pool for BATCH_SIZE_uniform.
S_id_uniform = np.array(grid['grid_id'].unique(), dtype=int)

def create_batch(batch_tensor=batch_tensor, labels=labels,
                 sample_ids_real=S_id_real, sample_ids_random=S_id_random,
                 sample_ids_uniform=None,
                 return_transf=False,
                 _size_padding=None,
    ...
    # default to the full-grid pool so existing call sites (e.g. the window-search
    # helper) that don't pass sample_ids_uniform still work
    if sample_ids_uniform is None:
        sample_ids_uniform = S_id_uniform
    ...
    for b in range(BATCH_SIZE):
        ...
        elif b < BATCH_SIZE_real + BATCH_SIZE_fill + BATCH_SIZE_random:
            s_id = int(np.random.choice(sample_ids_random)) if len(sample_ids_random) > 0 \
                   else int(np.random.choice(sample_ids_real))
        else:
            s_id = int(np.random.choice(sample_ids_uniform)) if len(sample_ids_uniform) > 0 \
                   else int(np.random.choice(sample_ids_random))
...
class Net(nn.Module):
    def __init__(self):
        ...
        # wide-context branch: unchanged from model 4, receptive field (49x49) much larger
        # than the input window (18x18)
        ...
        # local branch: receptive field = 1 (pure 1x1 convs, no spatial mixing), a small
        # nonlinear function of a cell's own raw features only. Added to the context branch's
        # output so the final logit is grounded in the cell's own characteristics regardless
        # of how much the wide-context branch smooths over the surrounding neighborhood.
        self.local = nn.Sequential(
            nn.Conv2d(in_channels=nc, out_channels=2*nc, kernel_size=1, bias=True),
            nn.LeakyReLU(),
            nn.Conv2d(in_channels=2*nc, out_channels=1, kernel_size=1, bias=True),
        )

    def forward(self, x):
        return self.context(x) + self.local(x)
...
num_distinct_train_uniform = int(len(S_id_uniform) * frac_train_uniform)
...
sample_train_uniform = list(np.random.choice(a=S_id_uniform,size=num_distinct_train_uniform,replace=False))
sample_train_uniform.sort()
...
if num_distinct_train_uniform < len(S_id_uniform):
    sample_eval_uniform = list(set(S_id_uniform) - set(sample_train_uniform))
else:
    sample_eval_uniform = S_id_uniform
sample_eval_uniform.sort()
...
for epoch in range(curr_epoch, bound_epochs):
    ...
    for i in range(ITERS):
        ...
        data = create_batch(sample_ids_real=sample_train_real,
                            sample_ids_random=sample_train_random,
                            sample_ids_uniform=sample_train_uniform)
        inputs, labels = data
        ...
        # evaluation sample:
        if frac_train_real < 1 or frac_train_random < 1 or frac_train_uniform < 1:
                ...
                with torch.no_grad():
                    for eval_i in range(100):
                        eval_inputs, eval_labels = create_batch(sample_ids_real=sample_eval_real,
                                                                ...
                                                                sample_ids_uniform=sample_eval_uniform)
                        if use_cuda and torch.cuda.is_available():
...
def visualize_batch(batch_tensor, labels, n_samples=4, feature_names=None):
    ...
    for b in range(n_samples):
        ...
        if b < BATCH_SIZE_real:
            ...
            category = 'RANDOM (candidate)'
        else:
            category = 'UNIFORM (any grid cell)'
