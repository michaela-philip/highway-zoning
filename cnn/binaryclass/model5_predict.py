...
# saved_model_filename = 'binaryclass/bc_model5.tar'  # path to saved model for prediction
...
with torch.no_grad():
    for batch_start in range(0, len(all_ids), BATCH_SIZE):
        ...
        # model 5: single-window prediction. create_batch_predict already centers each
        # cell's own dedicated window exactly on that cell (rel_row = rel_col = window // 2,
        # no jitter), so the logit at the window's own center position is this cell's
        # prediction -- no averaging across the ~324 overlapping windows a cell would
        # otherwise also appear (off-center) in, as model 4 did.
        window = 2 * size_padding + size_potential
        center = window // 2
        ...
        for b in range(actual_bs):
            if patch_origins[b] is None:
                continue
            grid_scores[batch_ids[b]].append(float(logits[b, center, center]))
...
filename_out = f'predicted_activation-model5_{cell_width}_{date}.csv'
