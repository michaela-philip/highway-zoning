...
def create_grid(zoning, centroids, geology, census, state59, state40, us59, us40, interstate, gridsize, city_sample, zoning2 = None, grid_0 = 1):
    ...
    census = gpd.sjoin(census, zoning)
    print('census and zoning joined')
    ...
    if 'index_right' in grid.columns:
        grid = grid.drop(columns=['index_right'])
    grid = grid.merge(place_highways(grid, state59, state40, us59, us40, interstate)[['grid_id', 'hwy_59', 'hwy_40', 'dist_to_hwy']],
                            on='grid_id',how='left')
    print('highways added to grid')

    # difference hwy indicator at the grid level
    grid['hwy'] = grid['hwy_59'] - grid['hwy_40']
    grid['hwy'] = np.where(grid['hwy'] < 0, 0, grid['hwy'])
    if 'index_right' in census.columns:
        census = census.drop(columns=['index_right'])
...
output.to_pickle('data/output/household_sample.pkl')