...
sweep_values = ind_sample['logit_normalized'].quantile([0.50, 0.75, 0.80, 0.90, 0.95, 0.99]).tolist()
...
export_single_regression(table, caption = 'Effect of Exposure to Black Homeowners on Highway Placement', label = 'tab:outside_corridor_black_homeowners', notes = notes, leaveout = leaveout_except(columns, keep=keep), widthmultiplier = 0.6)
...
LO, HI = ind_sample['black_homeowners_access'].quantile([0.40, 0.80])
...
table = predicted_outcomes_from_fit(model, ind_sample, x_vars, columns, link = 'log', eval_at = 'ame', black_values = [LO, HI], black_labels = ['Low Exposure', 'High Exposure'], sweep_var = 'logit_normalized', sweep_label = 'Est. Highway Suitability', sweep_values = sweep_values, sweep_interactions = LOGIT_INTERACTIONS)
