...
export_predicted_outcomes_table(table, caption = 'Predicted Highway Construction - Suitability Sweep', label = 'tab:predicted_outcomes/black_share_sweep', notes = notes, column_labels = sweep_cols)
...
"and can be interpreted as average marginal effects. Standard errors are computed via the delta method from the spatial covariance matrix. The Protection Effect is the difference in predicted rates between Residential and Non-Residential areas at each level of exposure to Black Residents. *** p < 0.01, ** p < 0.05, * p < 0.1" 
table = predicted_outcomes_from_fit(model, ind_sample, x_vars, columns, link = 'logit', eval_at = 'ame', black_values = [ind_sample['dem_access'].quantile(0.25), ind_sample['dem_access'].quantile(0.75)], black_labels = ['Low Exposure', 'High Exposure'])
export_predicted_outcomes_table(table, caption = 'Predicted Highway Construction - Linear Spline', label = 'tab:predicted_outcomes/black_share_logitinteraction', notes = notes)
...
export_predicted_outcomes_table(table, caption = 'Predicted Highway Construction - Linear Spline and Varying Suitability', label = 'tab:predicted_outcomes/black_share_sweep_logitinteraction', notes = notes, column_labels = sweep_cols)
