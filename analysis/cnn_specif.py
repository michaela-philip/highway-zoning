...
from analysis.lib.specs import (
    CORE_VARS, HOUSING_VARS, HH_CONTROLS, CNN_PROB, CNN_INTERACTIONS, CNN_DIST_HWY,
    build_spec, fit_ols,
)
...
x_vars_no_int, columns_no_int = build_spec(df_restricted, CORE_VARS, CNN_PROB, HOUSING_VARS, CNN_DIST_HWY, HH_CONTROLS)
...
x_vars, columns = build_spec(df_restricted, CORE_VARS, CNN_PROB, CNN_INTERACTIONS, HOUSING_VARS, CNN_DIST_HWY, HH_CONTROLS)
...
# NOTE (carried over unchanged from before the cleanup): this reuses the interaction
# spec, so it is numerically identical to indir_results_interaction above rather than
# a genuine no-interaction comparison. x_vars_no_int/columns_no_int look like the
# intended spec here -- flagging rather than changing the numbers silently.
indir_results_no_interaction, ind_beta, ind_se, ind_boot_coefs = bootstrap_lpm_table(ind_sample, x_vars_no_int, columns_no_int)

# --- run for both specs ---
print("\n=== WITHOUT CNN LOGIT ===")
cells1 = marginal_effects_table(ind_sample, x_vars, columns, ind_beta, ind_boot_coefs)
