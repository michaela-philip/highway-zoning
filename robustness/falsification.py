...
ind_sample = compute_characteristic_access(ind_sample, 'share_homeowners', 'homeowners_access', decay_m = 600, max_dist_m = 3000)
...
LOGIT_INTERACTIONS = sweep_interactions_spec(ind_sample, 'homeowners_access', 'logit_above_knot', 'High Suitability')
...
table = table.rename(index={
    ...
    'Black x CNN Logit': 'Exposure to Homeowners x Est. Highway Suitability',
    'Residential x Black x CNN Logit': 'Residential x Exposure to Homeowners x Est. Highway Suitability',
})
...
LO, HI = ind_sample['homeowners_access'].quantile([0.25, 0.75])
predicted_outcomes_from_fit(model, ind_sample, x_vars, columns, link = 'logit', eval_at = 'ame', black_values = [LO, HI], black_labels = ['Low Homeowner Access', 'High Homeowner Access'], sweep_var = 'logit_normalized', sweep_label = 'Est. Highway Suitability', sweep_values = sweep_values, sweep_interactions = LOGIT_INTERACTIONS)