...
sweep_values = ind_sample['logit_normalized'].quantile([0.10, 0.25, 0.50, 0.75, 0.90]).tolist()
...
decay_m = {300: None, 600: None, 1200: None, 1800: None}
for decay in [300, 600, 1200, 1800]:

    ind_sample = compute_characteristic_access(ind_sample, 'share_black', 'dem_access', decay_m = decay, max_dist_m = 3000)
    ...
    model_decay = fit_logit_firth_conley(ind_sample, x_vars, columns, y_var='hwy', cutoff_m=1500)
    table_decay = format_regression_results(model_decay)
    decay_m[decay] = table_decay
...
export_multiple_regressions(decay_m, caption = 'Robustness Check: Varying Decay Parameters for Exposure to Black Population', label = 'tab:robustness/decay_parameter', leaveout = leaveout_except(columns, keep = keep), notes = notes)
